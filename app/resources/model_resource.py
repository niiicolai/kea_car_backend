...
class ModelBaseResource(BaseModel):
    carmodel_name: str = Field(..., examples=["iX M60", "F-150 Lightning", "e-tron GT"])
    price: float = Field(..., examples=[9990.95, 690.69, 990.99])
    
    model_config = ConfigDict(from_attributes=True)
    
    @field_validator('carmodel_name')
    def validate_carmodel_name(cls, value: str) -> str:
...
class ModelUpdateResource(ModelCreateOrUpdateResource):
    ...
    carmodel_name: str | None = Field(None, examples=["iX M60", "F-150 Lightning", "e-tron GT"])
