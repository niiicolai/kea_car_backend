...
def get_all(
    ...
    if not isinstance(brand_repository, BrandRepository):
        raise TypeError(f"brand_repository must be of type BrandRepository, not {type(brand_repository)}")
...
def get_by_id(repository: ModelRepository, model_id: str) -> ModelReturnResource:
    if not isinstance(repository, ModelRepository):
        raise TypeError(f"repository must be of type ModelRepository, not {type(repository)}")
    if not isinstance(model_id, str):
        raise TypeError(f"model_id must be of type str, not {type(model_id)}")
